...
export function formatField(f: Cell[][]): string {
  ...
  const colorPerProduct = (uniq(f.flat().filter(Boolean)) as ProductCode[]).reduce((acc, p, i) => {
  ...
  return f.reduce((acc, row) => {
    return acc + row.reduce((acc, cell, c) => {
      return acc + (cell ? bracketIt(colorIt(cell)) : bracketedNonePlaceholder).padEnd(maxLengthPerColumn[c], " ");
    }, '') + '\n';
  }, '');
}